...
st.title("PhotosynQ Studio v0.2")
...
with st.sidebar:
    ...
    st.info("Se il CSV contiene Treatment/Trattamento/Group/Condition, l'app usa quello riga-per-riga. Questi campi sono fallback.")
    experiment = st.text_input("Esperimento", value="Maize HA experiment")
    treatment = st.text_input("Trattamento fallback", value="")
    sample_id = st.text_input("Sample ID fallback", value="")
    plant_id = st.text_input("Plant / pot ID fallback", value="")
    replicate = st.text_input("Replica fallback", value="")
    notes = st.text_area("Note fallback", value="", height=90)
